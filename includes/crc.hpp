...
const mpz_class appendCRC(mpz_class n)
{
    uint8_t* res;
    size_t len = utils::getRawMpz(&res, n);

    uint32_t crc32c =  0; 
    crc32c = calculate_crc32c(crc32c,(unsigned char*) res, len);
    memcpy(res+len, &crc32c, 4);

    mpz_class r = utils::setRawMpz(res, len+4);    
    return r;

}

const bool validateCRC(mpz_class n)
{
    uint8_t* res;
    size_t len = utils::getRawMpz(&res, n);
    ...
    uint32_t crc32c =  0; 
    crc32c = calculate_crc32c(crc32c,(unsigned char*) res, len-4);
    
    return (crc32c == value);
}

const mpz_class removeCRC(mpz_class n)
{
    uint8_t* res;
    size_t len = utils::getRawMpz(&res, n);
